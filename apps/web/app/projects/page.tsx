...
export default function ProjectsPage() {
  ...
  async function handleDelete(id: string, name: string) {
    if (!session || !confirm(`Delete project "${name}"? This cannot be undone.`)) return;
    try {
      await apiFetch(`/projects/${id}`, { method: "DELETE" }, session.accessToken);
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {loading ? (
            ...
            projects.map((project) => (
              <div key={project.id} className="group relative rounded-md border border-line bg-white p-5 transition-colors hover:border-brand">
                ...
                {session?.user?.role === "ADMIN" && (
                  <button
                    ...
                    className="absolute bottom-3 right-3 rounded-md p-1.5 text-muted opacity-0 transition-opacity group-hover:opacity-100 hover:bg-red-50 hover:text-red-600"
